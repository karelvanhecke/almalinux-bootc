...
ARG LOCKFILE=almalinux-bootc-lock.amd64.json
COPY . /src
WORKDIR /src
RUN --mount=type=cache,target=/workdir --mount=type=bind,rw=true,src=.,dst=/buildcontext,bind-propagation=shared rpm-ostree compose image \
 --image-config almalinux-bootc-config.json --cachedir=/workdir --format=ociarchive --initialize --lockfile=${LOCKFILE} ${MANIFEST} /buildcontext/out.ociarchive
